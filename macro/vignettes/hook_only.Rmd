...
for (main_idx in 1:10) {
  cat(paste("Running step", main_idx))
  modules <- step_mainloop(modules, observer, 1, dump_if)
...
aggregate(bm$Bites, by = list(Location=bm$Location), FUN=sum)
ggplot(bm, aes(Bites)) + geom_histogram()
```
```{r}
bh <- observer$bloodmeal_human[[day_str]]
ggplot(bh, aes(times)) + geom_histogram()
```
```{r}
day_str <- "1"
