...
How do we parameterize and initialize the state of the mosquito module? We need to choose initial values for $(M, Y, Z)$ We start with knowing the EIP, the daily survival, and the emergence rate. While we know the biting rate, $a$, we don't know the EIR yet.

For starters, the total population comes from a geometric series, where we apply mortality in the first step. That makes $M=\lambda p / (1-p)$.
```{r}
b <- 1/10
p <- 0.9
m <- 1-p
...
```{r}
make_tosolve <- function(parameters) {
  tosolve <- function(b) {
    lambda <- parameters$lambda  # emergence per day
    p <- parameters$p  # survival.
    M0 <- lambda / (1 - p)
    ...
    x <- c(M0 * 0.6, M0 * 0.2, M0 * 0.2)
    for (i in 1:1000) {
      x <- A %*% (x + c(lambda, 0, 0))
    }
    abs(sum(x[2:3]) / sum(x) - f)
  }
}
f <- 0.2
b <- (1 - p) * f / (1 + f)  # biting rate.
optim(b, make_tosolve(0.2), method = "Brent", lower = 0, upper = 1)

```
Try again, but use the actual matrix.
```{r}
b <- 1 / 100
p <- 0.9
EIP <- 3
